...
void disp_cur_pos() {
  ...
  // if(!move_or_zero)
  // {
    sprintf_P(public_buf_l, PSTR("X:%smm"), dtostrf(current_position.x, 1, 1, str_1));//  cur_pos
    lv_label_set_text(labelX, public_buf_l);

    sprintf_P(public_buf_l, PSTR("Y:%smm"), dtostrf(current_position.y, 1, 1, str_1));//  cur_pos
