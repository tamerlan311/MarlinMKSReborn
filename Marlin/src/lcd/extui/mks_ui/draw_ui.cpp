...
temp_value_state temp_value;
printflow_state printflow_speed;
...
lv_font_t gb2312_puhui11 = gb2312_puhui32;
void gCfgItems_init() {
  ...
  gCfgItems.pausePosX         = 5;
  gCfgItems.pausePosY         = 5;
  ...
  gCfgItems.PLA_EXT1 = 200;
  gCfgItems.PLA_BED = 60;
  gCfgItems.ABS_EXT1 = 240;
  gCfgItems.ABS_BED = 110;
  gCfgItems.encoder_enable = true;
  gCfgItems.button_sound = true;
  gCfgItems.endstop_sound = false;
  gCfgItems.print_completion_tone = true;
  gCfgItems.no_filament_tone = true;
  gCfgItems.babystep_data = 0;
  gCfgItems.add_full = false;
  gCfgItems.del_full = false;
  gCfgItems.Layout_stop_data  = 0;
  gCfgItems.Layout_stop_bottom_data  = 0;
  gCfgItems.save_layer_stop_num  = 0;
  gCfgItems.save_disp_layer_stop_num  = 0;
  ...
  if (gCfgItems.disp_rotation_180 != rot) {
    gCfgItems.disp_rotation_180 = rot;//0x00
    update_spi_flash();//將數據寫入flash
  }
...
void ui_cfg_init() {
  uiCfg.curTempType         = 0;//0為擠出頭 1為熱床，需要"HAS_HEATED_BED"
  uiCfg.extruderIndex       = 0;//擠出頭編號
  uiCfg.stepHeat            = 10;//溫度 增加/減少 數值
  uiCfg.leveling_first_time = false;
  uiCfg.para_ui_page        = false;
  uiCfg.extruStep           = uiCfg.eStepMed;//擠出步距
  uiCfg.extruSpeed          = uiCfg.eSpeedL;//擠出速度
  uiCfg.move_dist           = 1;//軸步距
  uiCfg.moveSpeed           = 1000;//移動速度
  uiCfg.stepPrintSpeed      = 10;//打印速度 增加/減少 數值
  ...
  #if ENABLED(MKS_WIFI_MODULE)//WiFi模塊數據配置
    memset(&wifiPara, 0, sizeof(wifiPara));//WiFi基礎信息
    memset(&ipPara, 0, sizeof(ipPara));//IP信息
  ...
  uiCfg.filament_loading_time = (uint32_t)((gCfgItems.filamentchange_load_length * 60.0f / gCfgItems.filamentchange_load_speed) + 0.5f);//物料安裝時間
  uiCfg.filament_unloading_time = (uint32_t)((gCfgItems.filamentchange_unload_length * 60.0f / gCfgItems.filamentchange_unload_speed) + 0.5f);//物料卸載時間
}
...
lv_style_t tft_style_label_preheat;
lv_style_t tft_style_bmp_ready;
lv_style_t tft_style_preHeat_scr;
lv_style_t tft_style_preHeat_label;
lv_style_t tft_style_preHeat_label_WHITE;
lv_style_t tft_style_preHeat_label_BLACK;
lv_style_t tft_style_preHeat_label_GRAY;
lv_style_t tft_style_preHeat_WHITE_CIRCLE;
lv_style_t tft_style_label_big_black;
lv_style_t tft_style_preHeat_GREEN;
lv_style_t tft_style_button_GREEN;
lv_style_t tft_style_preHeat_YELLOW;
lv_style_t tft_style_button_YELLOW;
lv_style_t tft_style_filament_YELLOW;
lv_style_t tft_style_preHeat_BLUE;
lv_style_t tft_style_button_BLUE;
lv_style_t tft_style_button_BLUE_WHITE;
lv_style_t tft_style_button_RED;
lv_style_t tft_style_level_gray;
lv_style_t tft_style_wifi_WHITE;
lv_style_t label_dialog_white;
lv_style_t label_dialog_black;
...
lv_style_t z_step_style, z_step_re_style;
...
void tft_style_init() {
  ...
  tft_style_label_rel.text.color      = LV_COLOR_TEXT;//LV_COLOR_TEXT
  ...
  // lv_font_roboto_22.line_height = 10;          /*The maximum line height required by the font*/
  // lv_font_roboto_22.base_line = 0;             /*Baseline measured from the bottom of the line*/

  lv_style_copy(&tft_style_label_preheat, &lv_style_scr);
  tft_style_label_preheat.text.font       = &TERN(HAS_SPI_FLASH_FONT, gb2312_puhui32, lv_font_roboto_22);//TERN(HAS_SPI_FLASH_FONT, &gb2312_puhui11, LV_FONT_DEFAULT);
  tft_style_label_preheat.text.letter_space = 0;
  tft_style_label_preheat.text.line_space = 2;
  tft_style_label_preheat.text.color = LV_COLOR_TEXT;

  // lv_font_roboto_22.line_height = 30;          /*The maximum line height required by the font*/
  // lv_font_roboto_22.base_line = 0;             /*Baseline measured from the bottom of the line*/
  // gb2312_puhui32.line_height = 10;
  lv_style_copy(&tft_style_bmp_ready, &lv_style_scr);
  tft_style_bmp_ready.text.font       = &TERN(HAS_SPI_FLASH_FONT, gb2312_puhui32, lv_font_roboto_22);//TERN(HAS_SPI_FLASH_FONT, &gb2312_puhui11, LV_FONT_DEFAULT);
  // tft_style_bmp_ready.text.letter_space = 0;
  // tft_style_bmp_ready.text.line_space = 10;
  tft_style_bmp_ready.text.color = LV_COLOR_TEXT;
  
  // lv_font_roboto_22.line_height = 26;          /*The maximum line height required by the font*/
  // lv_font_roboto_22.base_line = 6;             /*Baseline measured from the bottom of the line*/
  // gb2312_puhui32.line_height = 21;

  lv_style_copy(&tft_style_preHeat_scr, &tft_style_scr);
  tft_style_preHeat_scr.body.main_color = LV_COLOR_WHITE;//LV_COLOR_WHITE
  tft_style_preHeat_scr.body.grad_color = LV_COLOR_WHITE;
  tft_style_preHeat_scr.body.border.color = LV_COLOR_WHITE;
  tft_style_preHeat_scr.body.border.width = 0;
  tft_style_preHeat_scr.body.border.opa = 0;

  lv_style_copy(&tft_style_preHeat_GREEN, &tft_style_scr);
  tft_style_preHeat_GREEN.body.main_color = LV_COLOR_MAKE(0x18,0xa2,0x64);//LV_COLOR_WHITE
  tft_style_preHeat_GREEN.body.grad_color = LV_COLOR_MAKE(0x18,0xa2,0x64);

  lv_style_copy(&tft_style_button_GREEN, &tft_style_preHeat_GREEN);
  tft_style_button_GREEN.body.radius = 10;

  lv_style_copy(&tft_style_preHeat_YELLOW, &tft_style_scr);
  tft_style_preHeat_YELLOW.body.main_color = LV_COLOR_MAKE(0xff,0xa2,0x10);//LV_COLOR_WHITE
  tft_style_preHeat_YELLOW.body.grad_color = LV_COLOR_MAKE(0xff,0xa2,0x10);
  tft_style_preHeat_YELLOW.body.radius = 10;

  lv_style_copy(&tft_style_button_YELLOW, &tft_style_preHeat_YELLOW);
  tft_style_button_YELLOW.body.radius = 10;
  tft_style_button_YELLOW.text.font = &gb2312_puhui32;

  lv_style_copy(&tft_style_preHeat_BLUE, &tft_style_scr);
  tft_style_preHeat_BLUE.body.main_color = LV_COLOR_MAKE(0x41,0x85,0xf7);//LV_COLOR_WHITE
  tft_style_preHeat_BLUE.body.grad_color = LV_COLOR_MAKE(0x41,0x85,0xf7);

  lv_style_copy(&tft_style_button_BLUE, &tft_style_preHeat_BLUE);
  tft_style_button_BLUE.body.radius = 10;

  lv_style_copy(&tft_style_button_BLUE_WHITE, &tft_style_button_BLUE);
  tft_style_button_BLUE_WHITE.body.main_color = LV_COLOR_MAKE(0xb5,0xb5,0xb5);
  tft_style_button_BLUE_WHITE.body.grad_color = LV_COLOR_MAKE(0xb5,0xb5,0xb5);

  lv_style_copy(&tft_style_button_RED, &tft_style_button_BLUE);
  tft_style_button_RED.body.main_color = LV_COLOR_MAKE(0xd9, 0x4d, 0x40);
  tft_style_button_RED.body.grad_color = LV_COLOR_MAKE(0xd9, 0x4d, 0x40);
  tft_style_button_RED.text.color = LV_COLOR_WHITE;
  tft_style_button_RED.text.font = &gb2312_puhui32;

  lv_style_copy(&tft_style_wifi_WHITE, &tft_style_preHeat_GREEN);
  tft_style_wifi_WHITE.body.main_color = LV_COLOR_WHITE;
  tft_style_wifi_WHITE.body.grad_color = LV_COLOR_WHITE;
  tft_style_wifi_WHITE.text.color = LV_COLOR_BLACK;

  lv_style_copy(&tft_style_level_gray, &tft_style_preHeat_GREEN);
  tft_style_level_gray.body.main_color = LV_COLOR_MAKE(0xec, 0xec, 0xec);
  tft_style_level_gray.body.grad_color = LV_COLOR_MAKE(0xec, 0xec, 0xec);
  tft_style_level_gray.text.color = LV_COLOR_BLACK;

  lv_style_copy(&tft_style_label_big_black, &tft_style_preHeat_scr);
  tft_style_label_big_black.text.color = LV_COLOR_BLACK;

  lv_style_copy(&tft_style_preHeat_label, &tft_style_preHeat_scr);
  tft_style_preHeat_label.text.color = LV_COLOR_BLACK;
  tft_style_preHeat_label.text.font       = &TERN(HAS_SPI_FLASH_FONT, gb2312_puhui32, lv_font_roboto_22);
  


  lv_style_copy(&tft_style_preHeat_label_WHITE, &tft_style_preHeat_label);
  tft_style_preHeat_label_WHITE.text.color = LV_COLOR_WHITE;

  lv_style_copy(&tft_style_preHeat_label_BLACK, &tft_style_preHeat_label);
  tft_style_preHeat_label_BLACK.text.color = LV_COLOR_BLACK;
  tft_style_preHeat_label_BLACK.text.font = &gb2312_puhui32;

  lv_style_copy(&tft_style_preHeat_label_GRAY, &tft_style_preHeat_label_BLACK);
  tft_style_preHeat_label_GRAY.text.color = LV_COLOR_MAKE(0x6a, 0x6a, 0x6a);

  lv_style_copy(&label_dialog_white, &tft_style_preHeat_label_BLACK);
  label_dialog_white.text.color = LV_COLOR_WHITE;

  lv_style_copy(&tft_style_preHeat_WHITE_CIRCLE, &tft_style_preHeat_scr);
  tft_style_preHeat_WHITE_CIRCLE.body.radius = LV_RADIUS_CIRCLE;
  
  lv_style_copy(&tft_style_filament_YELLOW, &tft_style_button_RED);
  tft_style_filament_YELLOW.body.main_color = LV_COLOR_MAKE(0xff,0xd0,0x20);
  tft_style_filament_YELLOW.body.grad_color = LV_COLOR_MAKE(0xff,0xd0,0x20);

  
  lv_style_copy(&z_step_style, &lv_style_scr);
  z_step_style.body.main_color = lv_color_make(0xb5,0xb5,0xb5);
  z_step_style.body.grad_color = lv_color_make(0xb5,0xb5,0xb5);
  z_step_style.body.radius = 5;

  lv_style_copy(&z_step_re_style, &lv_style_scr);
  z_step_re_style.body.main_color = LV_COLOR_MAKE(0x41,0x85,0xf7);//LV_COLOR_MAKE(0x41,0x85,0xf7);
  z_step_re_style.body.grad_color = LV_COLOR_MAKE(0x41,0x85,0xf7);//LV_COLOR_MAKE(0x41,0x85,0xf7);
  z_step_re_style.body.radius = 5;

  lv_style_copy(&z_label_bg_style, &lv_style_scr);
  z_label_bg_style.body.main_color = LV_COLOR_WHITE;
  z_label_bg_style.body.grad_color = LV_COLOR_WHITE;
  z_label_bg_style.body.border.color = lv_color_make(0X62,0X62,0X62);
  z_label_bg_style.body.border.width = 3;
  z_label_bg_style.body.radius = 10;
}
...
char *creat_title_text() {
  ...
  void gcode_preview(char *path, int xpos_pixel, int ypos_pixel) {
      ...
      for (i = 0; i < 400; i += 2) {
        ...
        if (*p_index == 0x0000) *p_index = LV_COLOR_WHITE.full;//LV_COLOR_BACKGROUND.full; LV_COLOR_WHITE
      }
...
void GUI_RefreshPage() {
  ...
  switch (disp_state) {
    ...
    if (temps_update_flag) {
        temps_update_flag = false;
        disp_language(gCfgItems.language, 0);
    }
    break;
    case PRINTING_UI:
      if (temps_update_flag) {
...
void clear_cur_ui() {
  ...
  switch (disp_state_stack._disp_state[disp_state_stack._disp_index]) {
    ...
    case LAYER_STOP_UI:               lv_clear_layer_stop(); break;
...
void draw_return_ui() {
  if (disp_state_stack._disp_index > 0) {
    ...
    switch (disp_state_stack._disp_state[disp_state_stack._disp_index]) {
      case LAYER_STOP_UI:              lv_draw_layer_stop(); break;
      ...
      case TEMPSETTING_UI:              lv_draw_tempsetting(); break;
...
lv_obj_t* lv_screen_create(DISP_STATE newScreenType, const char *title) {
  ...
  lv_obj_set_style(scr, &tft_style_preHeat_scr);
  // lv_screen_set_style(scr, &tft_style_scr);
  lv_scr_load(scr);
  lv_obj_clean(scr);
...
lv_obj_t* lv_big_button_create(lv_obj_t *par, const char *img, const char *text, lv_coord_t x, lv_coord_t y, lv_event_cb_t cb, const int id, bool centerLabel) {
  ...
  if (gCfgItems.multiple_language) {
    ...
    if (centerLabel)
      ...
      lv_obj_align(label, btn, LV_ALIGN_IN_BOTTOM_LEFT, 0, BUTTON_TEXT_Y_OFFSET);// lv_obj_align(label, btn, LV_ALIGN_IN_BOTTOM_MID, 0, BUTTON_TEXT_Y_OFFSET);LV_ALIGN_IN_BOTTOM_LEFT
  }
...
void LV_TASK_HANDLER() {
  ...
  TERN_(HAS_GCODE_PREVIEW, disp_pre_gcode(8, 91));//(8, 36)
...
void lv_top_return_name(lv_obj_t *par, const char *img, lv_event_cb_t cb, const int id/*=0*/)
{
    lv_obj_t *imgtop = lv_img_create(par, NULL);
    lv_img_set_src(imgtop, "F:/bmp_preHeat_top.bin");
    lv_obj_set_pos(imgtop, 0, 0);
    lv_refr_now(lv_refr_get_disp_refreshing());

    lv_obj_t *buttonReturn = lv_imgbtn_create(par, "F:/bmp_preHeat_return.bin", cb, id);
    lv_obj_set_pos(buttonReturn, 6, 3);

    lv_obj_t *labelname = lv_label_create_empty(par);
    lv_label_set_text(labelname,img);
    lv_obj_set_pos(labelname, 69, 13);
    // lv_obj_align(labelname, buttonReturn, LV_ALIGN_OUT_BOTTOM_MID, 20, 0);
    
    lv_refr_now(lv_refr_get_disp_refreshing());
    lv_imgbtn_set_src_both(imgtop, "F:/bmp_preHeat_return.bin");
    lv_img_set_src(imgtop, "F:/bmp_preHeat_top.bin");
}

void lv_top_name(lv_obj_t *par, const char *img)
{
    lv_obj_t *imgtop = lv_obj_create(par, nullptr);
