...
void lv_draw_preHeat() {
  ...
  // #endif
#ifndef USE_NEW_LVGL_CONF
  // lv_big_button_create(scr, "F:/bmp_speed0.bin", preheat_menu.off, BTN_X_PIXEL * 2 + INTERVAL_V * 3, BTN_Y_PIXEL + INTERVAL_H + titleHeight, event_handler, ID_P_OFF);//关电机
  // lv_big_button_create(scr, "F:/bmp_preHeat_return.bin", "", 6, 3, event_handler, ID_P_RETURN);//返回
  ...
  // lv_group_add_obj(g,buttonReturn);
  // if (TERN0(HAS_ROTARY_ENCODER, gCfgItems.encoder_enable)){
  //   lv_group_add_obj(g, buttonspeed0);
  //   lv_group_add_obj(g, buttonReturn);
  // }
#else
